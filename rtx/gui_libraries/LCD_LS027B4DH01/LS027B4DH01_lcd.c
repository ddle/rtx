...
void LCD_DrawLine_old(uint8_t Xpos, uint16_t Ypos, uint16_t Length, uint8_t Direction)
{
  uint32_t i = 0;
...
{
	return (lcd_buffer[Ypos][(Xpos >> 3) + 1] & (0x01 << (7 - (Xpos & 0x07)))) ? (LCD_COLOR_BLACK): (LCD_COLOR_WHITE);
}
